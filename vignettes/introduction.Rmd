...
bosc = simulate_experiment(n_sub = 14, 
                           n_trials = 40, 
                           osc_params = c(0.5, .05, 4.5, 0), 
                           phase_jitter = c(pi/8, pi/4), 
                           freq_jitter = c(0, 1),
                           amplitude_jitter = c(0, .02)
                           )
bosc = generate_surrogates(bosc,
                           n_surr = 1000)

bosc = detrend_bosc(bosc, order = 1)
bosc = scale_bosc(bosc, method = "z")
# bosc = window_bosc(bosc, method = "hann")
#bosc = pad_bosc(bosc, method = "zero")

bosc = fft_bosc(bosc, overwrite = T)
bosc = test_fft(bosc, alpha_amp = .05/10, overwrite = T)

plot_complex(bosc, freqs = seq(2.5, 6.25, 1.25))
plot_spectrum(bosc)
plot_phase(bosc)
