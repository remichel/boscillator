...
plot_phase <- function(bosc, freqs = unique(bosc$data$ss$real$fft$f)) {
  ...
  l = bosc$data$ss$real$fft %>%
    ...
    dplyr::mutate(length = Mod(.data$complex),
  ...
  tmpPlot = ggplot2::ggplot(d, ggplot2::aes(x = .data$phase, y = 1)) +
    ...
    ggplot2::geom_segment(ggplot2::aes(x = .data$phase, xend = .data$phase, y = 0, yend = 1), size = 1, color = "grey40", alpha = 1,
                          arrow = ggplot2::arrow(type = "closed", length = ggplot2::unit(yMax/20, "npc")))+
    # mean unit phase vectors
    ggplot2::geom_segment(data = l, ggplot2::aes(x = .data$phase, xend = .data$phase, y = 0, yend = length),size = 2.25, color = "black",
                          arrow = ggplot2::arrow(type = "closed", length = ggplot2::unit(yMax/20, "npc")))+
